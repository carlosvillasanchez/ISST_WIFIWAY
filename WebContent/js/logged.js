...
function showCache(){
	  for (var i = 0; i < cache.length; i++) {
		    ...
		    $("#cache").append("<button onclick='cacheFunction("+lat+", "+lng+")' style='text-align: center;' class='list-group-item list-group-item-action list-group-item-secondary' >Latitud: "+lat + ", Longitud: " + lng +"</button>");
	  }
}
function cacheFunction(latc, lngc){
	petition(latc, lngc);
    map.setZoom(17);
    var latLng = {lat: latc, lng: lngc};
    addMarker(latLng);
}
function petitionCache(){
	var user = $('#user').val();
	$.ajax({
		  method: "GET",
		  //dataType: "json",
		  url: "http://localhost:8080/WifiWay/CacheServlet?email=" + user,
		  success: function(data) {
            console.log("SUCCESS");
            console.log(data);
            cache = data;
            showCache();//DEBERIAMOS HACERLO CON UNA PROMESAPERO NO PASA NA
        }
	});
}
function petition(lat, lng){
	...
	$.ajax({
		  method: "GET",
		  //dataType: "json",
		  url: "http://localhost:8080/WifiWay/PointServlet?lng=" + lng + "&lat=" + lat + "&email=" + user,
		  success: function(data) {
              console.log("SUCCESS");
              console.log(data);
              wifi = data;
