	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if(email != null) {
			...
			if (usuario != null) {
				...
				System.out.println(usuario.getPuntos()[0]);
				UserDAOImplementation.getInstance().updateUser(usuario);
		...
		*/
		...
		for(int i=0; i<wifis.length; i++) {
			if(!aux.contains(wifis[i].getSsid())){
				aux.add(wifis[i].getSsid());
			}
		}
		
		Wifi wifiFinal[] = new Wifi[aux.size()];
		int T = 0;
		
		for (int i=0; i<wifis.length; i++) {
			if(aux.contains(wifis[i].getSsid())){
				aux.remove(wifis[i].getSsid());
				wifiFinal[T] = wifis[i];
				T++;
			}
		}
		...
		for(int i = 0; i < wifiFinal.length; i++) {
			JsonObjectBuilder wifi = Json.createObjectBuilder()
	                .add("SSID", wifiFinal[i].getSsid())
	                .add("MAC", wifiFinal[i].getMAC())
	                .add("SignalStrength", (minimum + (int)(Math.random()*45)))
	                .add("Type", wifiFinal[i].getType())
	                ;
			array.add(wifi);
